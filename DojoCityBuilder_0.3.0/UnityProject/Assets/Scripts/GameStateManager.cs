...
public class GameStateManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (cameraController != null)
        {
            ...
            LogDebug("Camera controller disabled in Awake: " + cameraController.enabled);
        }
        else
        {
            LogDebug("Warning: Camera controller reference is missing in Awake");
        }
    ...
    private void Update()
    {
        // Testing shortcut - press P to print debug information
        if (Input.GetKeyDown(KeyCode.P))
        {
            PrintDebugState();
        }

        // Testing shortcut - press L to force start game
        if (Input.GetKeyDown(KeyCode.L))
        {
            LogDebug("TEST: Manually forcing game start");
            StartGame();
        }
    }
    ...
    {
        LogDebug("=== GAME STATE DEBUG ===");
        LogDebug($"gameStarted: {gameStarted}");
        LogDebug($"currentPlayerName: {currentPlayerName}");
        LogDebug($"Camera controller reference exists: {cameraController != null}");
        
        if (cameraController != null)
        {
            LogDebug($"Camera controller enabled: {cameraController.enabled}");
            LogDebug($"Camera position: {cameraController.transform.position}");
        }
        
        LogDebug($"Game UI reference exists: {gameUI != null}");
        if (gameUI != null)
        {
            LogDebug($"Game UI active: {gameUI.activeSelf}");
        }
    ...
    public void StartGame()
    {
        ...
        if (startScreenManager != null && startScreenManager.playerNameInput != null && string.IsNullOrEmpty(currentPlayerName))
        {
        ...
        // Enable camera controls - this is critical!
        if (cameraController != null)
        {
            LogDebug("About to enable camera controller");
            cameraController.enabled = true;
            LogDebug("Camera controller enabled state is now: " + cameraController.enabled);
            
            // Force active state through reflection if needed
        ...
        else
        {
            ...
            if (cameraController != null)
            {
                LogDebug("Found camera controller on second attempt");
                cameraController.enabled = true;
                SetCameraActive(true);
                LogDebug("Camera controller enabled: " + cameraController.enabled);
            }
    ...
    // Helper method to force-set camera active through reflection if needed
    private void SetCameraActive(bool active)
    {
        if (cameraController == null) return;

        // Check if there's an "isActive" field to set directly
        var field = typeof(CameraController).GetField("isActive", 
                        System.Reflection.BindingFlags.NonPublic | 
                        System.Reflection.BindingFlags.Instance);
        if (field != null)
        {
            field.SetValue(cameraController, active);
            LogDebug($"Set camera isActive = {active} via reflection");
        }

        // Check if there's a SetActive method we can call directly
        var method = typeof(CameraController).GetMethod("SetActive", 
                        System.Reflection.BindingFlags.NonPublic | 
                        System.Reflection.BindingFlags.Instance);
        if (method != null)
        {
            method.Invoke(cameraController, new object[] { active });
            LogDebug($"Called camera SetActive({active}) via reflection");
        }
    }
    ...
    private IEnumerator InitializeGameCoroutine()
    {
        ...
        else
        {
            ...
            {
                cameraController.enabled = true;
                LogDebug("Camera controller enabled in InitializeGameCoroutine");
            }
            else
            {
                LogDebug("Warning: Camera controller reference is missing in InitializeGameCoroutine");
            }
