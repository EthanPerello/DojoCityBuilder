...
public class ControlsManager : MonoBehaviour
{
    ...
    private void Update()
    {
        // Testing shortcut - press T to trigger continue
        if (Input.GetKeyDown(KeyCode.T))
        {
            Debug.Log("[ControlsManager] TEST: Manually triggering continue button");
    ...
    public void ShowControlsPanel()
    {
        if (controlsPanel != null)
        {
            ...
            Debug.Log("[ControlsManager] Showing controls panel");
        }
        else
        {
            Debug.LogError("[ControlsManager] Controls panel reference is missing!");
        }
    }
    ...
    // Public test method that can be called from elsewhere if needed
    public void TestContinueToGame()
    {
        Debug.Log("[ControlsManager] TestContinueToGame() called directly");
        OnContinueButtonClicked();
    }
}