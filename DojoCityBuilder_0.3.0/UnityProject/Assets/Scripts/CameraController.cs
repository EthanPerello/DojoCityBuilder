...
public class CameraController : MonoBehaviour
{
    ...
    private bool hasLoggedActiveState = false;

    void Awake()
    {
        Debug.Log("[CameraController] Awake called, enabled state: " + enabled);
    }

    void OnEnable()
    {
        Debug.Log("[CameraController] OnEnable called");
    }

    void Start()
    {
        Debug.Log("[CameraController] Start method called, enabled state: " + enabled);
        cameraRotation = transform.eulerAngles; // Store initial rotation
    }

    void Update()
    {
        // Add this once to detect when the controller becomes active
        if (!hasLoggedActiveState && enabled)
        {
            Debug.Log("[CameraController] Camera controller is now enabled and active");
            hasLoggedActiveState = true;
        }

        // For testing - press K to print debug info
        if (Input.GetKeyDown(KeyCode.K))
        {
            Debug.Log("[CameraController] DEBUG: Camera is enabled: " + enabled);
            Debug.Log("[CameraController] DEBUG: Camera position: " + transform.position);
            Debug.Log("[CameraController] DEBUG: Camera rotation: " + transform.eulerAngles);
        }
    ...
    void HandleMovement()
    {
        ...
        if (moveX != 0 || moveY != 0)
        {
            Debug.Log($"[CameraController] Input detected: Horizontal = {moveX}, Vertical = {moveY}");
        }
